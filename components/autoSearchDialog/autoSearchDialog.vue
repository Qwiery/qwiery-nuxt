			...
			<div class="fixed inset-0 overflow-y-auto">
				<div class="flex min-h-full items-center justify-center px-4 py-8">
					<TransitionChild as="template" enter="duration-300 ease-out" enter-from="opacity-0 scale-95" enter-to="opacity-100 scale-100" leave="duration-200 ease-in" leave-from="opacity-100 scale-100" leave-to="opacity-0 scale-95">
						<DialogPanel class="panel max-h-[60vh] border-0 p-0 rounded-lg overflow-visible w-6/12 max-w-4xl text-black dark:text-white-dark">
							<!--TopRight Icon-->
							...
							<div class="p-5">
								<div class="flex bg-primary-dark-light px-1 pb-2 mb-2 rounded">
									<auto-search class="focus" @query="onQuery"></auto-search>
								</div>
								<div class="datatable"></div>

								<div class="flex justify-end items-center mt-8">
									<div class="text-primary text-xs">TAB to select, ENTER to execute.</div>
								</div>
							</div>
						</DialogPanel>
					</TransitionChild>
...
<script setup lang="ts">
	...
	const emit = defineEmits<{
		(e: "onQuery", queryPath: string[]): void;
	}>();
</script>
